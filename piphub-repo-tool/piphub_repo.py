...
_piphub_repo_completion() {
    ...
    local commands="install uninstall push build update outdated verify sync list use add remove edit info ping search config status doctor bootstrap self-update history export import completion"
    if [[ $cword -eq 1 ]]; then
...
""",
    ...
    # completion
    comp = sub.add_parser("completion", help="Bash tab completion")
    comp.add_argument("--shell", default="bash")
